...
//    protected function load_catalog_category_uuid(): void
//    {
...
//    }
    ...
    public function set_catalog_category_uuid(string $uuid): void
    {
        $this->catalog_category_uuid = $uuid;
    }

    protected function _before_get_catalog_category_uuid(): void
    {
        $category_class = static::CONFIG_RUNTIME['class_dependencies'][CategoryInterface::class];
        if (
            $this->catalog_category_id
            && !$this->is_property_modified('catalog_category_uuid')
            && $this->record_data['catalog_category_uuid'] === null
        ) {
            $Category = new $category_class($this->catalog_category_id);
            $this->catalog_category_uuid = $Category->get_uuid();
        }
    ...
    public function load_images(): void
    {
        //if ($this->images === null) {
        if ($this->record_data['images'] === null) {
    ...
    protected function _before_get_images(): void
    {
        $this->load_images();
    }
    ...
    public function set_images(array $images): void
    {
        ...
        foreach ($images as $image) {
            ...
            //$this->images[] = str_replace($public_dir, '', $realpath);
            if ($realpath) { //may be bool if the file does not exist
                $this->record_data['images'][] = str_replace($public_dir, '', $realpath);
            }
        }
    }
    ...
    {
        $this->load_images();
        return parent::as_array(); // TODO: Change the autogenerated stub
    }
}